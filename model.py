...
TRAINING_DATA_PATHS = ['data/track1/driving_log.csv',
                       'data/track2/driving_log.csv',
                       'data/track3/driving_log.csv',
                       'data/track4/driving_log.csv',
                       'data/track5/driving_log.csv']
...
def get_generator(train_paths, val_paths, from_directory=False, batch_size=32, fit_sample_size=None):
    ...
    """
    Creates an image data generator for traning data and one for validation data.
    Left and Right images are added with an offset steering angle.
        ...
        :param fit_sample_size: The sample size used to fit the image generator.
                                If set to None, no fit will be done.
    ...
    '''Create feature value pairs for the left camera images by
       subtracting the offset from the steering angle.'''
    
    log_left = log[['left_img', 'steering_angle']].copy()
    log_left.loc[:, 'steering_angle'] -= SHIFT_OFFSET

    
    '''Create feature value pairs for the right camera images by
       adding the offset to the steering angle.'''
    
    log_right = log[['right_img', 'steering_angle']].copy()
    log_right.loc[:, 'steering_angle'] += SHIFT_OFFSET

    
    paths = pd.concat([log.center_img,\
                       log_left.left_img,\
                       log_right.right_img]).str.strip()
    
    values = pd.concat([log.steering_angle,\
                        log_left.steering_angle,\
                        log_right.steering_angle])
    ...
    if from_directory:
        return (datagen.flow_from_directory(paths.values,\
                                            values.values,\
                                            shuffle=True,\
                                            target_size=IMG_SIZE,\
                                            batch_size=batch_size),\
                
                val_datagen.flow_from_directory(val_log.center_img.values,\
                                                val_log.steering_angle.values,\
                                                shuffle=True,\
                                                target_size=IMG_SIZE,\
                                                batch_size=batch_size))
...
if __name__ == '__main__':
    ...
    rdi_train, rdi_val = get_generator(TRAINING_DATA_PATHS,\
